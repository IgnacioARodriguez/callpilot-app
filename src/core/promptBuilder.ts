...
export const buildPromptWithEvidence = (context: GlobalContext, userInput: string, evidence: EvidenceSelection): BuiltPrompt => {
  ...
  const structuredContract = context.activeMode === "live_coding"
    ? [
      ...
      "When screen_context contains visible starter code, solution.code must be the complete updated version of that visible code. Preserve visible function, method, class, parameter, and variable names unless the latest request explicitly asks to rename them.",
  ...
  const system = [
    ...
    "When visible starter code exists, return the smallest complete update to that code: keep visible signatures and names, change only the behavior needed by the latest visible requirement, and put any unknown value or input contract in assumptions.",

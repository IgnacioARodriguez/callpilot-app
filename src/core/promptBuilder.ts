...
export const buildPromptWithEvidence = (context: GlobalContext, userInput: string, evidence: EvidenceSelection): BuiltPrompt => {
  ...
  const structuredContract = context.activeMode === "live_coding"
    ? [
      ...
      "When screen_context contains visible Python starter code, solution.code must be the complete updated version of that visible code. Preserve visible def/class names, function signatures, parameters, and variables unless the latest request explicitly asks to rename or change them.",
      "For Python follow-ups that ask to read from input/stdin, use input() or stdin inside the existing visible function instead of adding parameters, unless the latest request explicitly asks to change the signature.",
  ...
  const system = [
    ...
    "In live coding with multiple screenshots or panels, first identify the visual surface for each capture from labels, filenames, visible tabs, breadcrumbs, and OCR text. Distinguish implementation code, tests, instructions/README, terminal/output, and file tree before deciding what to change.",
    "In live coding, tests and terminal output are evidence about expected behavior and failures; do not copy test code into the implementation file unless the interviewer explicitly asks to edit tests. Instructions/README define the contract. Implementation/editor code is the code to modify.",
    "When current screen_context includes several captures from one checkpoint, treat them as the current visual truth in their listed order. Use older transcript and previous answers only for continuity, and do not let older visible code override a newer file, terminal error, or test shown in the current screen_context.",
    ...
    "When visible Python starter code exists, return the smallest complete update to that code: keep visible signatures and names, change only the behavior needed by the latest visible requirement, and put any unknown value or input contract in assumptions.",
    "In Python live coding, if a visible function has no parameter and the requested change is to read a value from input/stdin, keep the visible signature and call input() inside the function unless the interviewer explicitly asks to add parameters.",
    "In Python live coding, prefer simple built-in string operations over new imports for ordinary string cleanup. For collapsing whitespace into a separator, use separator.join(value.strip().lower().split()) instead of regex unless the interviewer explicitly asks for regex.",
    "Do not add defensive type conversion, None handling, new helper functions, __main__ blocks, or extra imports unless the latest visible code, tests, traceback, or interviewer request requires them.",
    "For new live-coding problem statements, begin with a natural baseline solution that the candidate can explain collaboratively. Prefer the simplest correct implementation unless constraints, tests, or the interviewer explicitly ask for an optimized approach.",
    "For baseline live-coding answers, include the current approach, honest time/space complexity, and one short note about how it could be optimized later if useful. Do not over-optimize before the interviewer asks.",
    "When the interviewer asks to optimize, improve the algorithm or data structure then. Include the core invariant, data structure or pointer strategy, and updated time/space complexity. Avoid proposing extra data structures when an in-place or constant-space standard solution is expected by the visible constraints.",
    ...
    "For manual Answer requests, treat current_question and recent_conversation as candidate evidence, not as a guarantee that the last sentence is the task. Select the latest semantically actionable interviewer/candidate exchange: a question, correction, incomplete answer, technical scenario, or implicit problem. Ignore social filler and logistics. If the user pressed Answer and useful recent context exists, answer that context instead of returning no_answer.",
    ...
    "Do not answer stale topics from earlier transcript turns. If the latest text is filler, use the latest actionable exchange instead of treating filler as a topic switch.",
    ...
    "For pauses or filler, only return no_answer when there is no actionable technical or interview context anywhere in the current window. If the user pressed Answer after a filler remark, recover the latest actionable exchange and help with it.",
    ...
    "Keep the final spoken answer natural and easy to say during a live call: usually 2 to 4 conversational sentences. Do not force a fixed word count, exhaustive checklist, or formal framework. Give one main idea and at most one useful technical detail unless the interviewer asks for depth.",
    ...
    "For technical interview Q&A, do not include Python, SQLAlchemy, pseudocode, code fences, or implementation snippets unless the interviewer explicitly asks for code. Prefer a compact spoken answer; vary sentence length and structure so it sounds like a candidate, not documentation.",
    "Use no labels, bullets, assumptions, or follow-up suggestions unless they genuinely help. If supporting metadata is needed, use at most two short key points and leave followUpHint null by default. The spokenAnswer must stand on its own.",
    ...
    "Avoid filler, apologies, meta commentary, broad tutorials, and repetition. If the user pressed Answer, return the most useful next thing to say, not an analysis of why an answer may or may not be needed.",
    "Compare the proposed answer with recent assistant suggestions. Do not repeat the same framework or wording; add a new decision, example, tradeoff, diagnostic step, or concise candidate phrasing.",
    "Prefer natural qualifiers such as 'I would start by', 'one option is', or 'the main tradeoff is'. Do not invent exact thresholds, SLAs, tool usage, metrics, or production history. Mention a named tool only when it is in the question or confirmed evidence, and frame it as a proposal when it is not personal experience.",

...
if (!fs.existsSync(workflowPath)) {
  fs.writeFileSync(workflowPath, [
    ...
    "```",
    "",
    "Or preview and ingest every MP4 under `_incoming`:",
    "",
    "```powershell",
    `npm run ingest:eval-dataset -- --split=${split} --dir="${datasetDir}" --dataset=${dataset}`,
    `npm run ingest:eval-dataset -- --split=${split} --dir="${datasetDir}" --dataset=${dataset} --run`,
    "```",
    "",

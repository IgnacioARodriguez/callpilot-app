...
const sessions = [
  ...
  {
    sessionId: "system_design_url_shortener",
    mode: "system_design",
    profile: "headset_meet",
    duration_ms: 720000,
    full_duration_ms: 1800000,
    channels: ["system", "mic", "both"],
    description: "System design interview with scale changes, consistency tension, Redis pushback, multi-region scope, and final executive summary.",
    events: [
      {
        id: "initial_design",
        channel: "system",
        start_ms: 0,
        text: "Design a link shortener for ten million requests per day. Start by stating assumptions, read write ratio, and the main components.",
        critical_terms: ["link shortener", "ten million", "assumptions", "components"],
        topic: "url_shortener_initial_design",
      },
      {
        id: "candidate_baseline",
        channel: "mic",
        start_ms: 64000,
        text: "I would assume reads dominate writes, use an API service, base sixty two short codes, durable storage, and a cache in front of redirects.",
        critical_terms: ["reads dominate", "base sixty two", "durable storage", "cache"],
        topic: "candidate_initial_tradeoff",
        no_answer_expected: true,
      },
      {
        id: "scale_change",
        channel: "system",
        start_ms: 142000,
        text: "Now change the scale to five hundred million requests per day. Which part of the previous design stops holding up first?",
        critical_terms: ["five hundred million", "previous design", "stops holding up"],
        topic: "scale_change_bottleneck",
      },
      {
        id: "consistency_constraint",
        channel: "system",
        start_ms: 251000,
        text: "New constraint: product wants strongly consistent click counters, even though earlier we were leaning toward eventual analytics.",
        critical_terms: ["strongly consistent", "click counters", "eventual analytics"],
        topic: "consistency_tension",
      },
      {
        id: "candidate_acknowledges_tension",
        channel: "mic",
        start_ms: 336000,
        text: "That creates a tension. Strong counters are slower and harder across regions, while eventual analytics scales better for dashboards.",
        critical_terms: ["tension", "strong counters", "across regions", "eventual analytics"],
        topic: "candidate_tradeoff_memory",
        no_answer_expected: true,
      },
      {
        id: "redis_pushback",
        channel: "system",
        start_ms: 421000,
        text: "Why not simply use an atomic counter in Redis and call it done? Push back if that is too simplistic.",
        critical_terms: ["atomic counter", "Redis", "push back", "simplistic"],
        topic: "redis_counter_pushback",
      },
      {
        id: "multi_region_change",
        channel: "system",
        start_ms: 532000,
        text: "Final change: make it multi-region with low redirect latency. Do not invent an SLA or a number of regions.",
        critical_terms: ["multi-region", "low redirect latency", "do not invent", "number of regions"],
        topic: "multi_region_scope_change",
      },
      {
        id: "executive_summary",
        channel: "system",
        start_ms: 646000,
        text: "Give me a concise executive summary of the final link shortener design, including the multi-region tradeoff, click counter consistency, and why Redis alone is not enough.",
        critical_terms: ["executive summary", "link shortener", "multi-region tradeoff", "click counter consistency", "Redis alone"],
        topic: "final_design_summary",
        trigger_answer_ms: 658500,
      },
    ],
    expected: {
      latest_question_terms: ["link", "multi-region", "click counters", "Redis alone", "cross-region consistency"],
      stale_topic_forbidden_terms: ["Two Sum", "STAR", "Postgres index", "recent orders", "payment latency"],
      unsupported_behavioral_specifics: [
        "one million users",
        "Black Friday",
        "TechCorp",
